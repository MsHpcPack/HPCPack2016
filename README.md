...
**Note:** see **Pre-Requisites** section on this page before starting your deployment.
...
### Scenario 1: High-availability cluster for Windows workloads with Active Directory Domain
This template deploys an HPC Pack cluster with high availability for Windows HPC workloads in Active Directory Domain forest. The cluster includes one domain controller, three head nodes, one Database Server with SQL Server 2016 Standard version, and a number of **Windows compute nodes**.
...
### Scenario 2: High-availability cluster for Windows workloads
This template deploys an HPC Pack cluster with high availability for Windows HPC workloads. The cluster includes three head nodes, one Database Server with SQL Server 2016 Standard version, and a number of **Windows compute nodes**.
...
### Scenario 3: High-availability cluster for Linux workloads
This template deploys an HPC Pack cluster with high availability for Windows HPC workloads. The cluster includes three head nodes, one Database Server with SQL Server 2016 Standard version, and a number of **Linux compute nodes**.
...
### Scenario 4: Single head node cluster for Windows workloads
**Note:** Single head node cluster is for prototype/development purposes, NOT for production purposes.

This template deploys an HPC Pack cluster with one single head node and a number of Windows compute nodes. The head node is with local databases (SQL server 2016 Express version).
...
### Scenario 5: Single head node cluster for Linux workloads
**Note:** Single head node cluster is for prototype/development purposes, NOT for production purposes.

This template deploys an HPC Pack cluster with one single head node and a number of Linux compute nodes. The head node is with local databases (SQL server 2016 Express version).
